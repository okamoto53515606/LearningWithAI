...
| 2 | [VS Code インストール・リポジトリのクローン](#2-vs-code-のインストールとリポジトリのクローン) | プログラミング用エディタの準備 |
| 3 | [GitHub Copilot と VS Code の連携](#3-github-copilot-と-vs-code-の連携) | AI チャット機能の有効化 |
| 4 | [Terminal の使い方](#4-terminal-の使い方) | コマンド入力画面を開く方法 |
| 5 | [uv のインストール](#5-uv-のインストール) | Python 管理ツール |
| 6 | [nvm / Node.js / npx のインストール](#6-nvm--nodejs--npx-のインストール) | JavaScript 実行環境 |
| 7 | [Brave Search API の登録](#7-brave-search-api-の登録) | AI が画像検索できるようにする |
| 8 | [VOICEVOX のインストール](#8-voicevox-のインストール) | AI が声でしゃべれるようにする |
| 9 | [MCP の設定](#9-mcp-の設定) | AI とツールを繋ぐ設定ファイル |
| 10 | [Claude のテスト](#10-claude-のテスト) | 動作確認 |
| 11 | [設定ファイルのダウンロード](#11-設定ファイルのダウンロード) | くろーどちゃんの設定を取得 |
| 12 | [音声入力のセットアップ](#12-音声入力のセットアップ) | 声で話しかけられるようにする |
...
## 3. GitHub Copilot と VS Code の連携

### 3-1. Copilot ホーム画面に戻る
...
### 6-4. PowerShell の実行ポリシーと npx の確認
