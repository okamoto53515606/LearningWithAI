...
def load_save():
    if os.path.exists(SAVE_FILE):
        try:
            ...
            pass
    return {"current_q": 0, "score": 0, "total": len(QUESTIONS)}

def save_progress(current_q, score):
    with open(SAVE_FILE, "w") as f:
        json.dump({"current_q": current_q, "score": score, "total": len(QUESTIONS)}, f)
...
def make_sound(freqs, durations, volume=0.5):
    rate = 44100
    samples = []
...
sound_correct = make_sound([660, 880], [0.15, 0.25], volume=0.2)
sound_wrong   = make_sound([330, 220], [0.2, 0.3], volume=0.2)
...
def title_screen(save_data):
    clock = pygame.time.Clock()
    has_save = save_data["current_q"] > 0
    btn_new  = pygame.Rect(WIDTH//2 - 180, 340, 360, 64)
    btn_cont = pygame.Rect(WIDTH//2 - 180, 430, 360, 64) if has_save else None
    ...
    while True:
        ...
        screen.blit(t3, (WIDTH//2 - t3.get_width()//2, 260))
        draw_button(screen, btn_new, "はじめから", BLUE, btn_new.collidepoint(mx, my))
        if has_save:
            draw_button(screen, btn_cont,
                        f"{save_data['current_q']+1}もんめから つづき",
                        GREEN, btn_cont.collidepoint(mx, my))
        pygame.display.flip()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit(); sys.exit()
            if event.type == pygame.MOUSEBUTTONDOWN:
                if btn_new.collidepoint(mx, my):
                    delete_save()
                    return 0, 0
                if has_save and btn_cont and btn_cont.collidepoint(mx, my):
                    return save_data["current_q"], save_data["score"]
        clock.tick(60)
...
def game_loop(start_q, start_score):
    clock = pygame.time.Clock()
    current_q = start_q
    score = start_score
    selected = None
...
def main():
    while True:
        save_data = load_save()
        start_q, start_score = title_screen(save_data)
        game_loop(start_q, start_score)
